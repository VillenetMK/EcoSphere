...
const fixture = `
  ...
  insert into private.control_audit_log(actor_user_id,actor_username,actor_role,pump_requested,pump_allow_wet_soil,pump_bypass_sensor_checks,created_at)
    values ('${cima}','fixture-cima','operator',true,true,true,now()-interval '1 day');
  ...
  test(`${removePermissions ? 'eliminación definitiva' : 'restauración'} posterior a Eureka: migración y controles SQL reales`, async t => {
    ...
    try {
      ...
      if (removePermissions) {
        // Only the unchanged controller-authentication implementation is replaced
        // by a fixture. The migration's real public sync wrapper is exercised below.
        await db.exec(`
          create table private.ecosystems(id integer primary key,strict_controller_protocol boolean);
          insert into private.ecosystems values(1,true);
          create function private.controller_gateway_access_allowed() returns boolean
            language sql as $$ select current_setting('role')='service_role' $$;
          ...
          ) returns table(fan_target boolean,led_target boolean,auto_mode boolean,
            pump_request bigint,pump_duration_ms integer,fan_power integer,
            led_power integer,secure_mode boolean,heartbeat_seq bigint,
            pump_authorized boolean,pump_expires_at_epoch bigint)
          language sql as $$
            select c.fan_target,c.led_target,c.auto_mode,c.pump_request,
              c.pump_duration_ms,c.fan_power,c.led_power,true,1::bigint,true,
              extract(epoch from now()+interval '15 seconds')::bigint
            from public.device_control c where id=1
          $$;
        `);
      ...
      if (removePermissions) {
        await t.test('elimina las tablas y los RPC de permisos; los campos activos ya no existen', async () => {
          for (const table of ['manual_watering_permissions','ai_permissions','ai_session_usage']) {
            assert.equal((await db.query('select to_regclass($1) as object',[`private.${table}`])).rows[0].object,null);
          }
          for (const fn of ['reserve_ai_session','ai_sensor_snapshot','ai_provider_key','my_ai_access','my_control_permissions']) {
            assert.equal((await db.query('select to_regprocedure($1) as object',[`public.${fn}()`])).rows[0].object,null);
          }
          await assert.rejects(db.exec('update public.device_control set pump_bypass_sensor_checks=true'),/does not exist/);
          await assert.rejects(db.exec('update public.device_control set pump_allow_wet_soil=true'),/does not exist/);
        });

        await t.test('el ESP32 conserva el contrato pero siempre recibe los bypass denegados', () => scenario(async () => {
          await db.exec('set local role service_role;');
          const response=(await db.query("select * from public.controller_sync('fixture','fixture',1)")).rows[0];
          assert.equal(response.pump_authorized,true);
          assert.equal(response.pump_allow_wet_soil,false);
          assert.equal(response.pump_bypass_sensor_checks,false);
          assert.equal(response.led_power,67);
          assert.equal(response.fan_power,24);
          assert.equal(response.pump_request,42);
        }));

        await t.test('el wrapper de sincronización conserva la comprobación del gateway', () => scenario(async () => {
          await db.exec('set local role anon;');
          await assert.rejects(db.query("select * from public.controller_sync('fixture','fixture',1)"),/controller edge gateway required/);
        }));
      } else {
      ...
      for (const [label,assignment,error] of denied) {
        await t.test(`CIMA vuelve a bloquear el riego con ${label}`, () => scenario(async () => {
          await db.exec(`${removePermissions ? '' : 'update private.manual_watering_permissions set allow_wet_soil_manual_watering=true;'}
