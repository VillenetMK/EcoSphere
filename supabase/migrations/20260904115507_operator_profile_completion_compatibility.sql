...
-- remove_viewer_role migrated every pending viewer to operator, while the
-- immutable completion implementation still accepted only pending/viewer.
-- Rebind the already rate-limited one-shot implementation to the final role
-- model so verified email registrations can finish without weakening identity
-- immutability, uniqueness handling, or the five-attempt quota.
create or replace function private.complete_user_profile_once_impl(
  p_username text,
  p_first_name text,
  p_last_name text,
  p_dni text,
...
begin
  ...
  if found then
    ...
    if v_existing.status = 'pending' and v_existing.role = 'operator' then
  ...
  values (
    ...
    v_dni, v_phone, v_auth_email, v_method, 'approved', 'operator'
  );
...
comment on function private.complete_user_profile_once_impl(
  text, text, text, text, text, text
) is
  'Completes a missing or pending operator profile once; approved identity remains immutable.';
