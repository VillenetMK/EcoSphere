...
-- Remove the temporary Eureka permissions, rather than retaining disabled grants.
-- Preserve users, telemetry, actuator settings, controller identity and audit history.

begin;
set local lock_timeout = '5s';

-- Cancel only a legacy exceptional request if one was reintroduced. Never issue
-- a pump command or change automatic/manual mode, LED power or fan power.
update public.device_control
set pump_expires_at = null
where pump_allow_wet_soil or pump_bypass_sensor_checks;

-- The normal command path no longer stores or consults exhibition overrides.
...
-- Keep the audit trigger compatible with the reduced control row.
...
begin
  ...
  insert into private.control_audit_log (
    ...
    pump_requested, pump_duration_ms
  )
  values (
...
-- Preserve the wire contract for installed ESP32 firmware. These two legacy
-- response fields are constant false and cannot grant a bypass. Controller
-- authentication, replay protection and diagnostic validation are unchanged.
...
-- Remove the API and grant/rolling-quota storage for the retired voice feature.
-- RESTRICT is deliberate: unexpected dependencies abort the entire migration.
drop function public.reserve_ai_session();
drop function public.ai_sensor_snapshot();
drop function public.ai_provider_key();
drop function public.my_ai_access();
drop function public.my_control_permissions();
drop table private.ai_session_usage;
drop table private.ai_permissions;
drop table private.manual_watering_permissions;

alter table public.device_control
  drop column pump_allow_wet_soil,
  drop column pump_bypass_sensor_checks;

-- These audit columns describe past actions only; their false defaults apply to
-- all new records. Retaining them preserves evidence of exhibition-era commands.
comment on column private.control_audit_log.pump_allow_wet_soil is
  'Historical Eureka override, retired; normal commands never set this flag.';
comment on column private.control_audit_log.pump_bypass_sensor_checks is
  'Historical Eureka override, retired; normal commands never set this flag.';
