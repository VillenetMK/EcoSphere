...
-- Restore the pre-Eureka operator contract (d247283, 2026-09-05).
-- Keep applied migration history, users, telemetry, audit rows and controller
-- identity intact. Additive columns remain for old installed clients/firmware.

begin;
set local lock_timeout = '5s';

-- Expire only pending exhibition overrides; never issue a new pump command.
...
set allow_wet_soil_manual_watering = false, updated_at = now()
where allow_wet_soil_manual_watering;

update private.ai_permissions
set enabled = false, updated_at = now()
where enabled;

-- Cached exhibition clients receive an explicit denial with the same shape.
create or replace function public.my_control_permissions()
returns table(allow_wet_soil_manual_watering boolean, allow_sensorless_manual_watering boolean)
language sql stable security invoker set search_path = ''
as $permission$ select false, false; $permission$;
revoke all on function public.my_control_permissions() from public, anon, service_role;
grant execute on function public.my_control_permissions() to authenticated;

-- This gate was widened to all approved users during Eureka, so merely clearing
-- the former allowlist would not disable it. Close the gate and its RPC grants.
create or replace function public.my_ai_access()
returns boolean language sql stable security invoker set search_path = ''
as $access$ select false; $access$;
...
revoke all on function public.reserve_ai_session() from public, anon, authenticated, service_role;
revoke all on function public.ai_sensor_snapshot() from public, anon, authenticated, service_role;
revoke all on function public.ai_provider_key() from public, anon, authenticated, service_role;
