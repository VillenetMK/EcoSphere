...
-- Ajuste final de la cuarentena: sólo la humedad del suelo depende de la
-- versión exacta cuyo filtro GPIO34 fue validado. El flotador binario conserva
-- su semántica fail-safe: low significa agua no confirmada y bloquea la bomba.
...
begin
  ...
  if p_has_telemetry then
    ...
    values (
      ...
      p_air_humidity,
      case
        when v_soil_telemetry_safe then p_soil_humidity
        else null::double precision
      end,
      p_light_lux,
      case
        when p_water_level is null then null::text
...
comment on function public.controller_sync(
  ...
  integer,
  integer
) is 'Authenticates the active ESP32, quarantines unsafe soil telemetry unless the validated 2.0.5+replaceable firmware is reported, updates heartbeat, and returns control targets.';
