...
-- Allow successive manual watering requests without system/operator cooldowns.
-- Preserve authentication, pulse limits, sensor checks and existing set-point quotas.
-- Replacing the function does not issue commands or change any stored control state.
...
begin
  ...
  case v_action
    ...
    when 'pump' then
      ...
      -- Manual pulses have no additional waiting period. Each request still
      -- requires current telemetry, a valid soil reading and sufficient water.
